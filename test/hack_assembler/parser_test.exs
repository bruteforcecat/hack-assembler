defmodule HackAssembler.ParserTest do
  ...
  test "parse c instruction with valid dest and comp" do
    ...
    assert {:ok, {:c, %{comp: "D|A", dest: "AMD", jump: nil}}} == Parser.parse_instruction("AMD=D|A")
  end

  test "parse c instruction with valid comp and jump" do
    assert {:ok, {:c, %{comp: "0", dest: nil, jump: "JMP"}}} == Parser.parse_instruction("0;JMP")
    assert {:ok, {:c, %{comp: "D+A", dest: nil, jump: "JGT"}}} == Parser.parse_instruction("D+A;JGT")
  end

  test "parse c instruction with valid dest, comp and jump" do
    assert {:ok, {:c, %{comp: "0", dest: "M", jump: "JMP"}}} == Parser.parse_instruction("M=0;JMP")
    assert {:ok, {:c, %{comp: "D-A", dest: "AMD", jump: "JLT"}}} == Parser.parse_instruction("AMD=D-A;JLT")
  end
